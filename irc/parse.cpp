...
int Server::parse(int index, std::string buffer)
{
	...
	else if (sender.get_registred() < 1)
	{
		send_err(421, sender, "You have not registered");
		sender.add_registred(-1);
	}
	else if (!comand.compare("USER")) 
	{
		if (!user(sender, buffer))
			sender.add_registred(1);
	}
	else if (!comand.compare("NICK"))
	{
		if (!nick(sender, buffer))
			sender.add_registred(1);
	}
	else if (sender.get_registred() < 3)
	{
		send_err(421, sender, "You have not registered");
	}
	else if (!comand.compare("WHO"))
		who(sender, buffer);
	else if (!comand.compare("PING"))
		ping(sender, buffer);
	else if (!comand.compare("PRIVMSG"))
